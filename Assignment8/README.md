...
**For this one, I'm not sure if I'm doing the dictionary thing properly**
...
data_dict = {'stimulusId':stimId,'subjectResponse':subjResp,'subjectAaccuracy':respAccuracy,'correctResponse':correctResp,'responsetime':respTime}
...
for block in range(nBlocks):
    ...
    for trial in range(nTrials):    
        ...
        # image identity being shown
        image_shown = faceStims[trial]
        stimId[block][trial] = image_shown
        print(stimId[block][trial])
        
        # correct response = position of image as either left or right
        if my_image.pos[0] == -480: # if image is on the left side
            correctResp[block][trial] = ['a'] # correct response is key 'a'
            print(correctResp[block][trial])
        elif my_image.pos[0] == 480: # if image is on the left side
            correctResp[block][trial] = ['l'] # correct response is key 'l'
            print(correctResp[block][trial])
            ...
            if fix_frames < frameN <= (fix_frames + image_frames): 
                ...
                if keys:
                    ...
                    if 'escape' in keys: # if someone wants to escape the experiment
                        ...
                        respTime[block][trial] = rt_clock.getTime() # collect subject response time here instead
                        subjResp[block][trial] = keys # collect subject response here instead 
                        print(subjResp[block][trial], respTime[block][trial])
                if frameN == (fix_frames + image_frames): #last frame for the image
                    print("End image frame =", frameN) #print frame number 
        ...
        #-collect accuracy for that trial
        if subjResp[block][trial] == (correctResp[block][trial]):
            respAccuracy[block][trial] = 1 # 1 = accurate response
        elif subjResp[block][trial] != (correctResp[block][trial]):
            respAccuracy[block][trial] = 2 # 2 = inaccurate response
        
        print('face image = ', stimId[block][trial], 'correct response = ', 
              correctResp[block][trial], 'subject response = ', subjResp[block][trial], 
              'subject accuracy = ', respAccuracy[block][trial])
