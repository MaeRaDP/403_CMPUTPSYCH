...
#directory info
filename = 'V2subject1_math_Nov282022.csv' #file with csv extension
main_dir = os.getcwd()
...
# dictionary stuff
keys = ["Block", "Trial", "Problem", "Correct Response", "Subject Response", "Subject Accuracy", "Response Time"]
values = [blocks, trials, prob, corr_resp, sub_resp, sub_acc, resp_time]
...
with open(filepath_address, 'w', newline='') as csvfile:
    fieldnames = ['Block', 'Trial', 'Problem', 'Correct Response', 'Subject Response', 'Subject Accuracy', 'Response Time']
    data_writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    data_writer.writeheader()
    data_writer.writerow(subjectData)
